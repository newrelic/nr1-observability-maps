...
export default class Sidebar extends React.PureComponent {
    render() {
        let { sidebarOpen, sidebarView, setParentState, mapData, mapConfig, selectedNode, height } = this.props;

        if (sidebarOpen) {
            return (
                <Rail attached internal position="right" style={{ width: "400px", marginTop: "60px", height: height }}>
                    <Segment className="map-sidebar" style={{ height: "100%" }}>
                        <span className="siderbar-header">{selectedNode}</span>
                        <Icon
                            name="close"
                            onClick={() => setParentState({ sidebarOpen: false, selectedNode: "" })}
                            style={{
                                float: "right",
                                cursor: "pointer",
                                paddingRight: "10px",
