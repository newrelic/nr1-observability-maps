...
export default class SidebarConnections extends React.PureComponent {
  ...
  render() {
    const { mapData, mapConfig, selectedNode, setParentState } = this.props;
    ...
    const renderConnectionsBlock = (title, data) => {
      const activeMenuItem =
        title === 'Incoming' ? incomingActiveMenuItem : outgoingActiveMenuItem;
                ...
                return (
                  <List.Item key={i}>
                    <Icon
                      ...
                      name={
                        setEntityDesign(conn[direction].entity.entityType).icon
                      }
                    />{' '}
                    &nbsp;&nbsp;
                    <Checkbox
    ...
    if (mapData && mapData.nodeData[selectedNode]) {
      const relationships = mapData.nodeData[selectedNode].relationships || [];
