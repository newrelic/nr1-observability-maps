...
export default class EditNode extends React.PureComponent {
    ...
    componentDidUpdate() {
        let { mapConfig, selectedNode } = this.props;
            ...
            if (loaded != selectedNode) {
                ...
                if (mapConfig.nodeData[selectedNode].hoverMetrics) {
                    this.setState({
                        ...
                        hm_1_ACC: mapConfig.nodeData[selectedNode].hoverMetrics[1].accountId,
                        hm_2_ACC: mapConfig.nodeData[selectedNode].hoverMetrics[2].accountId,
                ...
                if (mapConfig.nodeData[selectedNode].customAlert) {
                    this.setState({
                        ...
                        alertNrqlAcc: mapConfig.nodeData[selectedNode].customAlert.alertNrqlAcc,
                        alertCritical: mapConfig.nodeData[selectedNode].customAlert.alertCritical,
                        alertCriticalOperator: mapConfig.nodeData[selectedNode].customAlert.alertCriticalOperator,
                        alertWarning: mapConfig.nodeData[selectedNode].customAlert.alertWarning,
                        alertWarningOperator: mapConfig.nodeData[selectedNode].customAlert.alertWarningOperator,
                        alertHealthy: mapConfig.nodeData[selectedNode].customAlert.alertHealthy,
                        alertHealthyOperator: mapConfig.nodeData[selectedNode].customAlert.alertHealthyOperator
                    });
                }
            }
        }
    ...
    render() {
            ...
            return (
                <>
                    ...
                    <Button
                        positive
                        style={{ float: "right" }}
                        onClick={() => this.saveNrql(setParentState, mapConfig, selectedNode)}
                    >
                        <Icon
                            name="spinner"
                            loading
                        ...
                        return (
                            <Form.Group widths={16} key={i}>
                                <Form.Input
                                    ...
                                    onChange={(e, d) => this.setState({ [`mc_${i + 1}_NRQL`]: e.target.value })}
                                />
                                <Form.Select
                                    width={3}
                                    label="Chart"
                                    ...
                                    onChange={(e, d) => this.setState({ [`mc_${i + 1}_TYPE`]: d.value })}
                                />
                                <Form.Select
                                    width={4}
                                    label="Account"
                                    ...
                                    options={accountOptions}
                                    onChange={(e, d) => this.setState({ [`mc_${i + 1}_ACC`]: d.value })}
                                />
                            </Form.Group>
                        );
                    })}
        ...
        const renderIconSet = () => {
            let userIcons = this.props.userIcons.map(set => ({ key: set.id, value: set.id, text: set.id }));
                ...
                <>
                    ...
                    <Form.Group inline widths={"16"}>
                        <Form.Field
                            ...
                            onChange={(e, d) => this.saveNrql(setParentState, mapConfig, selectedNode, d.value)}
                        />
                    </Form.Group>
                    <br />
                </>
        ...
        const renderCustomAlerting = () => {
            ...
            let options = [
                { label: "Critical", op: "alertCriticalOperator", val: "alertCritical" },
            ...
            return (
                <>
                    ...
                    <Header as="h5">
                        Set a NRQL query that returns a single numeric value to assess, then set the value option as a
                        numeric.
                    </Header>
                    ...
                    <Form.Group widths={"16"}>
                        <Form.Input
                            ...
                            onChange={(e, d) => this.setState({ alertNrql: e.target.value })}
                        />
                        <Form.Select
                            width={"6"}
                            label="Account"
                            ...
                            <div key={i}>
                                ...
                                <Form.Group inline widths={"16"}>
                                    ...
                                    <Form.Input
                                        ...
                                        onChange={(e, d) => this.setState({ [option.val]: e.target.value })}
                                    />
                                </Form.Group>
                            </div>
                        );
                    ...
                    <Form>
                        <Form.Group>
                            <Form.Field
                                ...
                                onChange={(e, d) => this.setState({ selectedEditOption: d.value })}
                            />
                        ...
                        {selectedEditOption == "customAlertSeverity" ? renderCustomAlerting() : ""}
                    </Form>
                </Modal.Content>
