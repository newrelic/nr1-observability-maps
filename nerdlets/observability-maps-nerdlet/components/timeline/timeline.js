...
export default class TimelineView extends React.PureComponent {
    ...
    renderDeploymentDetails(event) {
        return (
            <div style={{ paddingBottom: "5px" }}>
                ...
                {event.changelog ? `Change Log: ${event.changelog}\n` : ""}
    ...
    renderTimeline(events, height) {
        return (
            <div style={{ overflowY: "scroll", maxHeight: height - 60 }}>
                <Timeline>
                    {events.map((event, i) => {
                        let eventType = event.violationId ? "alert" : "deployment";
                        let title = "";
                        ...
                        if (eventType == "deployment") {
                            ...
                            if (event.closedAt) {
                                let dateClosed = new Date(event.closedAt);
                                createdAt =
                                    createdAt +
                                    " - " +
                                    dateClosed.toLocaleDateString() +
                                    " " +
                                    dateClosed.toLocaleTimeString();
                            } else {
                        ...
                        return (
                            <TimelineEvent
                                ...
                                icon={icon}
                            >
                                {eventType == "deployment" ? this.renderDeploymentDetails(event) : ""}

                                <Button size="tiny" onClick={() => navigation.openStackedEntity(event.guid)}>
                                    <Icon name="external" />
                                    Entity
                                </Button>

                                {eventType == "alert" ? (
                                    <Button onClick={() => window.open(event.violationUrl, "_blank")}>
                                        <Icon name="external" />
                                        View Violation
                                    </Button>
                                ) : (
                                ...
                                {eventType == "deployment" ? (
                                    <Button onClick={() => window.open(event.permalink, "_blank")}>
                                        <Icon name="external" />
                                        View Deployment
                                    </Button>
                                ) : (
    ...
    render() {
        let { timelineOpen, data, setParentState, mapData, mapConfig, selectedNode, height } = this.props;
        ...
        if (data && data.nodes && data.nodes.length > 0) {
            for (let i = 0; i < data.nodes.length; i++) {
                if (data.nodes[i].recentAlertViolations && data.nodes[i].recentAlertViolations.length > 0) {
        ...
        let allEvents = [...recentAlerts, ...recentDeployments].sort((a, b) => b.timestamp - a.timestamp);

        if (timelineOpen) {
            return (
                <Rail attached internal position="right" style={{ width: "33%", marginTop: "60px", height: height }}>
                    <Segment className="map-sidebar" style={{ height: "100%" }}>
                        ...
                        {activeItem == "alerts" ? this.renderTimeline(recentAlerts, height) : ""}
                        {activeItem == "deployments" ? this.renderTimeline(recentDeployments, height) : ""}
                    </Segment>
                </Rail>
            );
        }
