...
export default class CreateMap extends React.PureComponent {
    ...
    render() {
        ...
        let { dataFetcher, handleChange, userMaps, accountMaps } = this.props
        userMaps = userMaps || []
        ...
        return (
            <Modal open={open} size="tiny" trigger={<Button onClick={()=>this.setState({open: true})} className="filter-button" icon="cog" />}>
                ...
                <Modal.Content>
                    ...
                    <Form>
                        <Form.Input
                            ...
                            fluid
                            // label='First name'
                            value={mapName} onChange={(e)=>this.setState({mapName:e.target.value})}
                            placeholder='Name...'
                        />
                        {/* <Form.Field>
                ...
                <Modal.Actions>
                    <Button disabled={mapNameError} positive onClick={()=>this.save(dataFetcher)}>Create</Button>
                    <Button style={{float:"left"}} negative onClick={this.close}>Close</Button>
                </Modal.Actions>
            </Modal>
        )
    }
